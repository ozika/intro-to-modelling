...
# Learning objectives {.section-title background-color="#2c3e50"}
...
df_p <- data.frame(
  x    = c(1.5,  4.5,             7.5,     4.5),
  y    = c(0.8,  0.8,             0.8,     2.1),
  label = c("DATA", "FREE PARAMETERS", "MODEL", "DESIGN"),
  desc  = c(
    "Trial-by-trial responses,\nreaction times, ratings, etc.",
    "Tune the model to an individual\n(e.g. learning rate α)",
    "Mathematical process that\ngenerates predictions",
    "Task structure, stimuli,\nreward schedule, timing"
  ),
  fill = c("#3d5a7a", "#5a7a3d", "#7a3d5a", "#7a6035")
)
ggplot(df_p) +
  geom_tile(aes(x = x, y = y, fill = I(fill)),
            width = 2.4, height = 0.9, colour = "white", linewidth = 2) +
  geom_text(aes(x = x, y = y + 0.15, label = label),
            colour = "white", fontface = "bold", size = 3) +
  geom_text(aes(x = x, y = y - 0.15, label = desc),
            colour = "white", size = 2, lineheight = 1) +
  # DATA <-> FREE PARAMETERS
  annotate("segment", x = 2.72, xend = 3.28, y = 0.8, yend = 0.8,
           arrow = arrow(ends = "both", length = unit(0.28, "cm")),
           colour = "#555555", linewidth = 1.5) +
  # FREE PARAMETERS <-> MODEL
  annotate("segment", x = 5.72, xend = 6.28, y = 0.8, yend = 0.8,
           arrow = arrow(ends = "both", length = unit(0.28, "cm")),
           colour = "#555555", linewidth = 1.5) +
  # FREE PARAMETERS <-> DESIGN (vertical)
  annotate("segment", x = 4.5, xend = 4.5, y = 1.27, yend = 1.63,
           arrow = arrow(ends = "both", length = unit(0.28, "cm")),
           colour = "#555555", linewidth = 1.5) +
  # DESIGN <-> DATA (diagonal)
  annotate("segment", x = 3.32, xend = 2.68, y = 1.68, yend = 1.27,
           arrow = arrow(ends = "both", length = unit(0.28, "cm")),
           colour = "#555555", linewidth = 1.5) +
  # DESIGN <-> MODEL (diagonal)
  annotate("segment", x = 5.68, xend = 6.32, y = 1.68, yend = 1.27,
           arrow = arrow(ends = "both", length = unit(0.28, "cm")),
           colour = "#555555", linewidth = 1.5) +
  xlim(0.1, 9) + ylim(0.2, 2.8) +
  theme_void()
```
...
ggplot(df_data, aes(x = trial)) +
  ...
  labs(x = "Trial", y = "Reward (0–100)",
       title = "Reward prediction task",
       subtitle = "On each trial a reward is drawn from a block-specific distribution") +
  theme_pres()
```

::: {.caption}
[**● True reward**]{style="color:#DC143C"} &emsp; | Dashed lines = reversals
:::
